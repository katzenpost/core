...
func TestEd25519ExpiredCertificate(t *testing.T) {
	...
	certificate, err := Sign(signingPrivKey, ephemeralPrivKey.PublicKey().Bytes(), "authority", expiration)
	assert.NoError(err)
...
func TestEd25519Certificate(t *testing.T) {
	...
	certificate, err := Sign(signingPrivKey, toSign, "authority", expiration)
...
func TestEd25519MultiSignatureCertificate(t *testing.T) {
	...
	certificate, err := Sign(signingPrivKey1, ephemeralPrivKey.PublicKey().Bytes(), "authority", expiration)
	assert.NoError(err)
...
func TestEd25519MultiSignatureOrdering(t *testing.T) {
	...
	// 1
	certificate1, err := Sign(signingPrivKey1, ephemeralPrivKey.PublicKey().Bytes(), "authority", expiration)
	assert.NoError(err)
	...
	// 2
	certificate2, err := Sign(signingPrivKey1, ephemeralPrivKey.PublicKey().Bytes(), "authority", expiration)
	assert.NoError(err)
	...
	// 3
	certificate3, err := Sign(signingPrivKey2, ephemeralPrivKey.PublicKey().Bytes(), "authority", expiration)
	assert.NoError(err)
